export const LABELS = {

    HOME: [
        "FoodBridge AI",
        "AI-Powered Food Redistribution System",
        "Reduce Food Waste. Feed Communities.",
        "Connect businesses with NGOs to redistribute surplus food efficiently using AI.",
        "Login",
        "Register",
        "Learn More",
        "Food redistribution, reimagined",
        "One shared mission:",
        "turn surplus into support for every community.",
        "Better food systems",
        "Connecting surplus food with the people who need it most.",
        "Reduce waste",
        "Make every good meal count by giving surplus food a meaningful next destination.",
        "Smart matching",
        "Use intelligent coordination to connect donors, volunteers, and recipient communities.",
        "Strengthen communities",
        "Create a reliable, dignified path from available food to local impact.",
        "better food, shared further.",
        "Reducing waste. Feeding communities.",
        "Home",
        "About",
        "How It Works",
        "Features",
        "Impact",
        "Language",
        "Get Started",
        "Explore Features",
        "Reduce Food Waste.",
        "Redistribute Surplus.",
        "Create Impact.",
        "Reduce Food Waste. Redistribute Surplus. Create Impact.",
        "FoodBridge AI connects food businesses with NGOs and recipient networks to identify surplus early, prevent avoidable food waste, and coordinate prompt, dignified redistribution through structured inventory management.",
        "Multi-Stakeholder Coordination",
        "Expiry Date Monitoring",
        "Structured Food Redistribution",
        "Surplus Inventory Coordination",
        "Platform Active",
        "Organic Whole Milk (50L)",
        "Dairy • 50 Units",
        "Expires in 18 hrs",
        "Bakery Surplus Assortment",
        "Bakery • 30 kg",
        "Claimed by City Kitchen",
        "Fresh Produce Batch",
        "Produce • 80 kg",
        "Scheduled for Pickup",
        "Community Food Recovery",
        "Inventory Tracking System",
        "About FoodBridge AI",
        "Bridging the Gap Between Surplus Food and Local Need",
        "Connecting surplus food with communities through practical technology and organized coordination.",
        "The Challenge of Food Waste",
        "Every day, supermarkets, restaurants, and caterers generate nutritious edible surplus that risks disposal due to tight expiry windows or lack of immediate logistics.",
        "The FoodBridge Solution",
        "FoodBridge AI provides clear inventory tracking, expiry date monitoring, and direct donation coordination, turning surplus food into community nourishment.",
        "Prevent Waste",
        "Shelf-life tracking and risk indicators help businesses identify surplus items before they reach expiration.",
        "Connect Surplus",
        "Direct digital connection between food businesses, registered NGOs, and local delivery partners.",
        "Measure Impact",
        "Donation records and reporting tools give donors and recipient organizations clear visibility into redistributed food.",
        "Step-by-Step Process",
        "How FoodBridge AI Coordinates Food Redistribution",
        "A simple, reliable workflow that turns surplus inventory into timely meals for communities.",
        "Track Inventory",
        "Log food items with expiry dates, categories, and batch details through direct entry, barcode scanning, or CSV upload.",
        "Monitor Expiry",
        "Configurable shelf-life timelines help flag perishable inventory approaching expiration dates.",
        "Identify Surplus",
        "Eligible inventory is flagged as surplus and organized into donation listings with category and quantity details.",
        "Coordinate Donation",
        "Registered NGOs review available surplus listings and submit requests matching their community needs.",
        "Schedule Pickup",
        "Coordinate pickup timing and logistics details between donor facilities and community partners.",
        "Track Impact",
        "Log completed handoffs and maintain transparent records of rescued food and community support.",
        "Platform Capabilities",
        "Built for Every Stakeholder in Food Recovery",
        "Core tools and workflows designed to make surplus food recovery practical, efficient, and accessible.",
        "Inventory Management",
        "Track stock quantities, units, categories, and storage conditions in a centralized catalog.",
        "Time-sensitive views and proactive indicators highlight perishables approaching expiry.",
        "Product Categorization",
        "Structured categorization across dairy, bakery, produce, prepared meals, and pantry goods.",
        "Waste Risk Assessment",
        "Early risk flagging helps staff identify surplus items in time to prioritize donation.",
        "Barcode & QR Support",
        "Camera-based scanning enables rapid item lookup and streamlined inventory logging.",
        "CSV & POS Import",
        "Upload product inventories in batches using standard CSV spreadsheets and POS files.",
        "Food Donation Coordination",
        "Turn surplus stock into accessible donation listings for registered community organizations.",
        "Role-Based Portals",
        "Dedicated dashboards for Businesses, NGOs, Delivery Drivers, Admins, and Individuals.",
        "Analytics & Accessibility",
        "View donation history and redistribution trends with support for 6 regional languages and PWA.",
        "Platform Roles",
        "Empowering Every Role in the Food Recovery Network",
        "Dedicated tools designed for donors, recipient organizations, delivery partners, and coordinators.",
        "Donor",
        "Commercial Businesses",
        "Supermarkets, restaurants, and caterers managing inventory, tracking expiry, and donating surplus food.",
        "Go to Business Portal",
        "Recipient",
        "NGOs & Food Banks",
        "Community kitchens and food relief organizations reviewing surplus listings, requesting items, and managing rations.",
        "Go to NGO Portal",
        "Logistics",
        "Delivery Partners",
        "Logistics volunteers and drivers managing pickup queues and recording completed distributions.",
        "Go to Delivery Portal",
        "Community",
        "Individual Donors",
        "Citizens and households listing surplus food and contributing directly to local hunger relief.",
        "Go to Individual Portal",
        "Oversight",
        "System Administrators",
        "Platform administrators managing user registrations, system oversight, and redistribution records.",
        "Go to Admin Portal",
        "Why It Matters",
        "Creating Environmental and Social Value Together",
        "Every redirected meal addresses hunger while preventing avoidable organic waste in landfills.",
        "Environmental Stewardship",
        "Diverting edible food from landfills helps reduce methane emissions and saves natural resources.",
        "Community Food Security",
        "Redirecting surplus food supplies nutritious meals to local shelters, food banks, and families.",
        "Corporate Responsibility",
        "Helps businesses cut waste disposal overhead while advancing meaningful community engagement.",
        "Responsible Food Logistics",
        "Promotes structured handling, clear pickup coordination, and transparent delivery logging.",
        "Join the Movement",
        "Turn Surplus Food Into Shared Impact.",
        "Join FoodBridge AI today. Whether you are a business looking to reduce waste, an NGO feeding communities, or an individual wanting to help.",
        "Create an Account",
        "Sign In to Your Dashboard",
        "Navigation",
        "Business Portal",
        "NGO & Food Banks",
        "Delivery Partner",
        "Individual Donor",
        "Admin Portal",
        "Register / Create Account",
        "Sign In to Portal",
        "FoodBridge AI. All rights reserved.",
        "Community Surplus Redistribution"
    ],

REGISTER: [

    "FoodBridge AI",

    "AI-Based Food Redistribution System",

    "Register",

    "Register As",

    "Business",

    "NGO",

    "Delivery Partner",

    "Admin",

    "Individual Donor",

    "Business Owner Name",

    "Business Name",

    "Business Type",

    "Supermarket",

    "Restaurant",

    "Bakery",

    "Cafe",

    "Hotel",

    "Cloud Kitchen",

    "Organization Name",

    "Registration Number",

    "Username",

    "Email",

    "Phone",

    "Password",

    "Already have an account?",

    "Login",

    "Registration Successful 🎉",

    "Registration Failed",

    "Join the movement",

    "A smarter network for rescuing food, reducing waste and strengthening communities.",

    "Less waste, more good",

    "Your impact journey starts here.",

    "Create your account",

    "Get started with FoodBridge AI in a few simple steps.",

    "Home",

    "Multi-Stakeholder Coordination",

    "Expiry Date Monitoring",

    "Structured Food Redistribution"

],

LOGIN: [

    "FoodBridge AI",

    "AI-Powered Food Redistribution System",

    "Login",

    "Username",

    "Password",

    "Login As",

    "Business",

    "NGO",

    "Delivery Partner",

    "Admin",

    "Individual Donor",

    "Don't have an account?",

    "Register",

    "Login Successful 🎉",

    "Invalid Credentials",

    "Smarter food impact",

    "Connect surplus food with the people and communities who need it most.",

    "Every meal matters",

    "Secure, simple and built for impact.",

    "Welcome back",

    "Sign in to continue creating a difference.",

    "Manage inventory, surplus food and donations.",

    "Receive and distribute donated food.",

    "Pickup and deliver food donations.",

    "Donate surplus food directly.",

    "Manage the complete FoodBridge platform."

],

    SIDEBAR: [
        "FoodBridge AI",
        "Dashboard",
        "Inventory",
        "Donations",
        "Transactions",
        "Analytics",
        "Barcode Scanner",
        "Settings",
        "Logout"
    ],

    NAVBAR: [ ],

    BUSINESS_DASHBOARD: [

    "Owner",
    "Business Type",
    "Welcome",
    "AI Food Redistribution Dashboard",

    "Expiry Alert",
    "Review Inventory",
    "Inventory Healthy",
    "No products require immediate attention.",

    "Products",
    "Near Expiry",
    "Donations",
    "Food Saved",

    "Add Product",
    "Upload CSV",
    "Scan Barcode",
    "Import From POS",

    "Recent Products",

    "Name",
    "Category",
    "Quantity",
    "Expiry",
    "Status",
    "Action",

    "No Products Yet",

    "Edit",
    "Delete",
    "Donate",

    "Donated",
    "Accepted",
    "Available",

    "Expired",
    "Critical",
    "Warning",
    "Safe",
    "product(s) will expire within the next",
    "days",
    "Delete this product?",

"Donate this product?",

"Product Donated Successfully 🎉",

"Donation Failed",

"CSV Uploaded Successfully 🎉",

"CSV Upload Failed",

"POS Inventory Synced Successfully 🎉",

"POS Import Failed"

],

    INVENTORY: [

    "Inventory",

    "Image",
    "Product",
    "Brand",
    "Barcode",
    "Category",
    "Risk",
    "Quantity",
    "Expiry",
    "Status",
    "Action",

    "Products",
    "Total Products",
    "High Risk",
    "Medium Risk",
    "Low Risk",

    "No Products",
    "No Image",

    "Locked",

    "High",
    "Medium",
    "Low",

    "Manage and monitor all your inventory products.",

    "Delete this product?",

    "Available"

],

   DONATIONS: [

    "My Donations",

    "Track all donated products and their current status.",

    "Total Donations",

    "Product",
    "Category",
    "Quantity",
    "Expiry",
    "Status",

    "Available",
    "Accepted",
    "Completed",
    "Donated",
    "Scheduled",
    "Delivered",
    "Pending",
    "Rejected",
    "Cancelled",

    "Action",

    "No Donations Yet"
],

    ANALYTICS: [

    "Analytics",

    "Analytics Dashboard",

    "Monitor inventory performance and donation insights.",

    "Total Products",
    "Near Expiry",
    "Donated",
    "Accepted",

    "Category Distribution",
    "Inventory By Category",
    "Inventory Overview",
    "Inventory Progress",
    "Quick Insights",

    "Donated Products",
    "Accepted Products",
    "Near Expiry Products",

    "Products Currently Available",
    "Products Donated",
    "Products Near Expiry",
    "Products Accepted",

    "Dairy",
    "Bakery",
    "Fruits",
    "Vegetables",
    "Others",

    "Products"

],

    TRANSACTIONS: [

    "Transaction History",

    "View all inventory activities and donation transactions.",

    "Total Transactions",

    "Date",

    "Product",

    "Action",

    "Status",

    "Added",

    "Donated",

    "Available",

    "Accepted",

    "Completed",

    "Scheduled",

    "Delivered",

    "Pending",

    "Rejected",

    "Cancelled",

    "📶 Offline Mode: Showing last synced transaction history.",

    "No Transactions Found"
],

BARCODE: [

    "Barcode / QR Scanner",

    "Scanned Barcode / QR",

    "Use Barcode",

    "Camera Error"

],

    SETTINGS: [

    "Business Settings",

    "Manage your business profile and account information.",

    "Username",
    "Owner Name",
    "Business Name",
    "Business Type",

    "Inventory",
    "Manage products efficiently.",

    "Donations",
    "Track donated products.",

    "Analytics",
    "Monitor business insights.",

    "Save Changes",

    "Profile Updated Successfully ✅"

],

    NGO_DASHBOARD: [

    "NGO Distribution Dashboard",

    "Welcome",

    "Active Donations",

    "Registration No:",

    "Service Area:",

    "Contact Person:",

    "Email:",

    "Available Donations",

    "Accepted Donations",

    "Scheduled Pickups",

    "Delivered Donations",

    "Notifications",

    "No notifications available.",

    "Business",

    "Product",

    "Category",

    "Quantity",

    "Expiry",

    "Status",

    "Action",

    "Accept Donation",

    "No Donations Available",

    "Available",

    "Accepted",

    "Scheduled",

    "Delivered",

    "Completed",

    "Donated",

    "Accept this donation?",

    "Donation Accepted Successfully 🎉",

    "Acceptance Failed",

    "new donation(s) available.",

    "donation(s) accepted.",

    "pickup(s) scheduled.",

    "donation(s) delivered successfully.",

],

NGO_SIDEBAR: [

    "FoodBridge AI",

    "Dashboard",

    "Available Donations",

    "Accepted Donations",

    "Pickup Schedule",

    "Donation History",

    "Beneficiaries",

    "Reports",

    "Profile",

    "Settings",

    "Logout"

],

NGO_AVAILABLE_DONATIONS: [

    "Available Donations",

    "Available",

    "View and accept food donations from businesses.",

    "Product",

    "Category",

    "Quantity",

    "Expiry",

    "Status",

    "Action",

    "Accept Donation",

    "No Donations Available",

    "Accept this donation?",

    "Donation Accepted Successfully 🎉",

    "Acceptance Failed",

    "Donated",

    "Accepted",

    "Scheduled",

    "Delivered",

    "Completed"

],

NGO_ACCEPTED_DONATIONS: [

    "Accepted Donations",

    "Available",

    "View all donations accepted by your NGO.",

    "Product",

    "Category",

    "Quantity",

    "Expiry",

    "Status",

    "Action",

    "Schedule Pickup",

    "No Donations Available",

    "Accepted",

    "Scheduled",

    "Delivered",

    "Completed",

    "Donated"

],

NGO_PICKUP_SCHEDULE: [

    "Scheduled Pickups",

    "Scheduled",

    "Product",

    "Quantity",

    "Pickup Date",

    "Pickup Time",

    "Volunteer",

    "Vehicle",

    "Status",

    "Action",

    "Confirm Pickup",

    "No Scheduled Pickups",

    "Pickup Confirmed Successfully 🚚",

    "Accepted",

    "Scheduled",

    "Delivered",

    "Completed"

],

NGO_DONATION_HISTORY: [

    "Donation History",

    "Product",

    "Category",

    "Quantity",

    "Volunteer",

    "Vehicle",

    "Status",

    "Completed",

    "No Completed Donations"

],

NGO_BENEFICIARIES: [

    "Beneficiaries",

    "Organizations receiving food through the FoodBridge AI platform.",

    "Organizations",

    "Meals Served",

    "Cities Covered",

    "Active NGOs",

    "Registered Beneficiaries",

    "Organization",

    "Category",

    "Contact Person",

    "Status",

    "Orphanage",

    "Old Age Home",

    "Homeless Shelter",

    "Community Kitchen",

    "Active",

    "Pending"

],

NGO_REPORTS: [

    "NGO Reports",

    "Meals Distributed",

    "Food Collected",

    "Completed Pickups",

    "CO₂ Saved",

    "Monthly Summary",

    "Month",

    "Donations",

    "Meals Served",

    "January",

    "February",

    "March"

],

NGO_PROFILE: [

    "NGO Profile",

    "Registration No:",

    "Address:",

    "Service Area:",

    "Contact Person:",

    "Phone:",

    "Email:",

    "Operating Hours:",

    "Edit Profile"

],

NGO_SETTINGS: [

    "NGO Settings",

    "Notifications",

    "Language",

    "Theme",

    "Email Alerts",

    "Enabled",

    "Disabled",

    "English",

    "Hindi",

    "Light",

    "Dark",

    "Save Settings"

],

DELIVERY_SIDEBAR: [

    "FoodBridge AI",

    "Dashboard",

    "Assigned Pickups",

    "Navigation",

    "Completed Deliveries",

    "Profile",

    "Settings",

    "Logout"

],

DELIVERY_DASHBOARD: [

    "Delivery Partner Dashboard",

    "Welcome Delivery Partner",

    "Delivery Partner",

    "Name:",

    "Vehicle Number:",

    "Phone:",

    "Status:",

    "AVAILABLE",

    "BUSY",

    "OFFLINE",

    "Assigned",

    "Out For Pickup",

    "Completed",

    "Distance Covered",

    "Notifications",

    "Live Driver Location",

    "You have new pickup assignments.",

    "You are currently delivering food.",

    "Great job! Deliveries completed successfully.",

    "No pending pickups. Waiting for next assignment.",

    "Km"

],

ASSIGNED_PICKUPS: [

    "Assigned Pickups",

    "Assigned",

    "Product",

    "Quantity",

    "Pickup Date",

    "Pickup Time",

    "Volunteer",

    "Vehicle",

    "Status",

    "Action",

    "No Assigned Pickups",

    "Scheduled",

    "Out For Pickup",

    "Delivered",

    "Completed",

    "Start Pickup",

    "Verify Pickup OTP",

    "Verify Delivery OTP",

    "Enter Pickup OTP",

    "Enter Delivery OTP",

    "Pickup Started Successfully 🚚",

    "Failed to Start Pickup",

    "Pickup OTP Verified",

    "Delivery Completed Successfully",

    "Verification Failed"

],

DELIVERY_NAVIGATION: [

    "Navigation",

    "Active Delivery",

    "Product",

    "Quantity",

    "Volunteer",

    "Vehicle",

    "Status",

    "Action",

    "No Active Deliveries",

    "Navigate",

    "Delivered",

    "Out For Pickup",

    "Completed",

    "Delivery Completed Successfully 🎉",

    "Failed"

],

COMPLETED_DELIVERIES: [

    "Completed Deliveries",

    "Completed",

    "Product",

    "Quantity",

    "Volunteer",

    "Vehicle",

    "Status",

    "No Completed Deliveries"

],

DELIVERY_PROFILE: [

    "Delivery Partner Profile",

    "Email:",

    "Phone:",

    "Vehicle Number:",

    "License Number:",

    "Experience:",

    "Years",

    "Status:",

    "Available",

    "Edit Profile"

],

DELIVERY_SETTINGS: [

    "Delivery Settings",

    "Notification Preference",

    "Enabled",

    "Disabled",

    "Language",

    "English",

    "Hindi",

    "Theme",

    "Light",

    "Dark",

    "Save Settings"

],

ADMIN_SIDEBAR: [

    "FoodBridge AI",

    "Dashboard",

    "Inventory",

    "Donations",

    "Transactions",

    "Analytics",

    "Settings"

],

    ADMIN_DASHBOARD: [

    "Admin Dashboard",

    "Monitor users, donations and deliveries across the platform.",

    "System Overview",

    "Businesses",

    "NGOs",

    "Products",

    "Accepted",

    "Total Users",

    "Delivery Partners",

    "Scheduled",

    "Delivered"

],

ADMIN_INVENTORY: [

    "Inventory Management",

    "View all products available in the system.",

    "Total Products",

    "Search Product...",

    "Product",

    "Category",

    "Quantity",

    "Status",

    "Expiry",

    "ID"

],
ADMIN_DONATIONS: [

    "Donations Management",

    "Monitor all donated food across the platform.",

    "Total Donations",

    "Accepted",

    "Scheduled",

    "Delivered",

    "Search Product...",

    "ID",

    "Product",

    "Business",

    "Category",

    "Quantity",

    "Status",

    "Expiry"

],
ADMIN_TRANSACTIONS: [

    "Transactions",

    "Complete history of all food donation transactions.",

    "Total Transactions",

    "Accepted",

    "Scheduled",

    "Delivered",

    "Search Product...",

    "ID",

    "Product",

    "Business",

    "Category",

    "Quantity",

    "Status",

    "Expiry Date",

    "Pending",

    "No transactions found."

],
ADMIN_ANALYTICS: [

    "Analytics Dashboard",

    "Monitor platform performance and donation insights.",

    "Total Businesses",

    "Total NGOs",

    "Products",

    "Delivery Partners",

    "Accepted",

    "Scheduled",

    "Delivered",

    "Pending",

    "Donation Status",

    "Platform Overview",

    "Donation Progress",

    "Businesses",

    "NGOs",

    "Delivery"

],
ADMIN_SETTINGS: [

    "Admin Settings",

    "Manage your profile and application settings.",

    "Admin Profile",

    "Username :",

    "Email :",

    "Role :",

    "Phone :",

    "Not Available",

    "Change Password",

    "Current Password",

    "New Password",

    "Confirm Password",

    "System Information",

    "Application :",

    "Version :",

    "Backend :",

    "Running",

    "Database :",

    "Connected",

    "Logout",

    "Click below to securely logout.",

    "Passwords do not match",

    "Password Changed Successfully",

    "Current password is incorrect"

],
INDIVIDUAL_SIDEBAR: [

    "Individual",

    "Dashboard",

    "Donate Food",

    "My Donations",

    "Settings"

],
INDIVIDUAL_DASHBOARD: [

    "Individual Donor Dashboard",

    "Welcome to FoodBridge AI.",

    "Total Donations",

    "Pending",

    "Accepted",

    "Scheduled",

    "Delivered"

],
DONATE_FOOD: [

    "Donate Food",

    "Food Name",

    "Category",

    "Quantity",

    "Expiry Date",

    "Pickup Address",

    "Contact Number",

    "Description",

    "Donation Submitted Successfully!",

    "Failed to submit donation."

],
MY_DONATIONS: [

    "My Donations",

    "Food",

    "Category",

    "Quantity",

    "Status",

    "Expiry",

    "Accepted",

    "Scheduled",

    "Delivered",

    "Pending"

],
INDIVIDUAL_SETTINGS: [

    "Individual Settings",

    "Profile",

    "Username:",

    "Email:",

    "Role:",

    "Individual Donor",

    "Change Password",

    "Old Password",

    "New Password",

    "Confirm Password",

    "Update Password",

    "Account Information",

    "Total Donations:",

    "Logout",

    "Passwords do not match",

    "Password Updated Successfully",

    "Failed to Update Password"

],
};